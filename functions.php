<?php
/**
 * RB Electrical Service functions and definitions
 *
 ...
 */
function rb_electrical_service_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on RB Electrical Service, use a find and replace
		* to change 'rb-electrical-service' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'rb-electrical-service', get_template_directory() . '/languages' );
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'rb_electrical_service_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'rb_electrical_service_setup' );
 ...
 */
function rb_electrical_service_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'rb_electrical_service_content_width', 640 );
}
add_action( 'after_setup_theme', 'rb_electrical_service_content_width', 0 );
 ...
 */
function rb_electrical_service_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'rb-electrical-service' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'rb-electrical-service' ),
...
add_action( 'widgets_init', 'rb_electrical_service_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
// function rb_electrical_service_scripts() {
// 	wp_enqueue_style( 'rb-electrical-service-style', get_stylesheet_uri(), array(), _S_VERSION );
// 	wp_style_add_data( 'rb-electrical-service-style', 'rtl', 'replace' );

// 	wp_enqueue_script( 'rb-electrical-service-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

// 	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
// 		wp_enqueue_script( 'comment-reply' );
// 	}
// }
// add_action( 'wp_enqueue_scripts', 'rb_electrical_service_scripts' );

// --------------

function rb_electrical_service_scripts() {

	wp_enqueue_style('rb-electrical-service-bootstrap-min', get_template_directory_uri() . '/assets/css/bootstrap.min.css', array(), '20151215');
	wp_enqueue_style('rb-electrical-service-animate', get_template_directory_uri() . '/assets/css/animate.min.css', array(), '20151215');
